...
This bubble chart visualises the distribution of total revenue for different entities in the network. Each bubble represents an entity, with the size of the bubble corresponding to the total revenue and the colour indicating the magnitude of the revenue. For example, the largest yellow bubble represents `Briggs-Wilson`, a company with a total revenue of approximately 310.6 million. 
...
print(g1)
```
This heatmap visualises the irregular patterns in revenue for different entities over time. Each cell represents a specific entity and year, with the numbers indicating the weight or frequency of revenue changes. The colour intensity highlights periods and entities with more significant variations. For instance, `Yang PLC` shows notable irregularities with higher weights in recent years, indicating fluctuating revenue patterns. 
...
print(g)
```
This network visualisation showcases the top 30 entities, highlighting their connections and interactions from 2005 to 2034. Larger nodes signify entities with more connections, illustrating their influence within the network. The separate cluster shows that `workfor` is separated from shareholdership and beneficialownership. 
