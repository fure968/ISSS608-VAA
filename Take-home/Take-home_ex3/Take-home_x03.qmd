...
print(combined_plot)
```
The dataset primarily consists of nodes categorized as 'Person,' with a count exceeding 50,000, indicating that individuals are the most common entities in the network. Companies are the next most prevalent node type, numbering around 5,000, showing significant but lesser representation compared to individuals. Nodes identified as 'CEO' are present but much fewer, reflecting a small proportion of individuals with this specific role. Other organization types, such as 'FishingCompany,' 'LogisticsCompany,' 'FinancialCompany,' 'NewsCompany,' and 'NGO,' are present in minimal numbers, highlighting their minority status in the dataset. The most common type of relationship is 'Shareholdership,' with over 35,000 connections, indicating ownership stakes in companies as a primary form of interaction. 'BeneficialOwnership' is the second most common relationship, with around 20,000 instances, showing significant connections where entities benefit from asset ownership. 'WorksFor' relationships also appear frequently, denoting employment links between individuals and organizations. 'FamilyRelationship' connections are the least represented, suggesting that familial ties are not as prominent as business and ownership relations in the dataset. Overall, the network focuses heavily on business and economic interactions.
...
Each node represents an entity, such as a company or individual, and the edges depict the relationships between them, including shareholdership, beneficial ownership, and employment links. This visualisation helps in understanding the complex web of interactions in the dataset, highlighting how entities are interconnected through various types of relationships.
...
This bar chart visualises the top names with multiple types of relationships in the network. The x-axis lists the top entities, while the y-axis represents the count of different relationship types. The chart highlights the entities with the highest number of connections, illustrating how some organisations or individuals are central to multiple types of interactions within the network. For example, Downs Group and Mills, Atkinson and Chavez have the highest number of relationships, indicating their significant roles in various types of network interactions.
...
This bubble chart visualises the distribution of total revenue for different entities in the network. Each bubble represents an entity, with the size of the bubble corresponding to the total revenue and the colour indicating the magnitude of the revenue. For example, the largest yellow bubble represents Briggs-Wilson, a company with a total revenue of approximately 310.6 million. This visualisation helps in identifying entities with significant financial influence within the network.
...
print(g1)
```
This heatmap visualises the irregular patterns in revenue for different entities over time. Each cell represents a specific entity and year, with the numbers indicating the weight or frequency of revenue changes. The colour intensity highlights periods and entities with more significant variations. For instance, Yang PLC shows notable irregularities with higher weights in recent years, indicating fluctuating revenue patterns. This visualisation helps in identifying entities with inconsistent revenue trends over time.
...
print(g)
```
This network visualisation showcases the top 30 entities, highlighting their connections and interactions from 2005 to 2034. Larger nodes signify entities with more connections, illustrating their influence within the network. The varied colours and connections provide insights into the temporal dynamics and key players within the network.
...
The scatter plot displays activity counts per year, categorised by BeneficialOwnership, Shareholdership, and WorksFor relationships. The y-axis represents the activity count, while the x-axis shows the year. Outliers, highlighted in blue, indicate entities with unusually high activity levels. Notably, `Stephens-Lopez` stands out with consistent high activity counts in the BeneficialOwnership category around 2030. Similarly, `Duncan-Williams` shows significant activity in the early 2020s in the same category. In Shareholdership, `Haas-Tran` and `Blair PLC` display peaks around 2030 and 2025, respectively.
...
``` 
This heatmap visualises outliers in activity counts for BeneficialOwnership and Shareholdership relationships over time. Each cell represents the activity count for a specific entity and year, with colour intensity indicating higher activity levels. Entities like `Wright LLC` and `Stephens-Lopez` show notable outliers in BeneficialOwnership, while `Woods Inc` and `Weaver-Price` display irregular activity in Shareholdership. 
