...
The graph I have chosen is from [here](https://isss608-zjho008.netlify.app/take-home_ex/take-home_ex01/take-home_ex01).
...
In this case violin graph can be great to visualise the output of Unit Price (\$per square foot) vs. total by property types.
...
```{r}
# Assuming `combined_ds` is your dataset
ggplot(combined_ds, aes(x = `Property Type`, y = `Unit Price ($ PSF)`, fill = `Property Type`)) +
