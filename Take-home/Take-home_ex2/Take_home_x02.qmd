...
colors <- c("slategray4", "darkolivegreen3", "darkslategray")

ggplot_object <- ggplot(data = combined_ds, aes(x = `Property Type`, y = `Unit Price ($ PSF)`, fill = `Property Type`)) +
  geom_violin(trim = FALSE, adjust = 1, alpha = 0.6, color = "gray30") +
  geom_boxplot(width = 0.3, fill = colors, outlier.shape = 10, outlier.color = "gray30", outlier.size = 2, alpha = 0.8) +  # Masculine colors for boxplots
  stat_summary(fun = median, geom = "text", aes(label = round(..y.., 2)), 
               color = "white", size = 3.5, vjust = 0, hjust = -0.5, fontface = "bold") +  # Bold and larger median text in white
  geom_hline(yintercept = overall_median, color = "firebrick4", size = 0.75, linetype = "dotted") +  # Dotted and thinner median line
  annotate("text", label = paste("Median: $", round(overall_median, 2)), x = Inf, y = overall_median, hjust = 1.1, vjust = 0, color = "white", size = 4, fontface = "bold", 
           bg = "firebrick4", box.colour = "darkred") +  # Label box for median line in masculine colors
  ...
  theme(axis.title = element_text(size = 10),
        plot.title = element_text(size = 12, face = "bold"),  # One size larger for the title
        plot.subtitle = element_text(size = 11),
        axis.text.x = element_text(size = 10, face = "bold"),  # Bold and one size up for X-axis labels
        plot.background = element_rect(fill = "gray20"),  # Dark background color
        panel.grid.major = element_line(color = "gray50"),  # Darker grid color
        legend.position = "none") +  # Completely hide the legend
  scale_y_continuous(breaks = seq(0, 6500, by = 500), limits = c(0, 6500))  # Extend Y-axis to 6500
