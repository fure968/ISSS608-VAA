...
## 1. Visualising and Analysing Text Data

1.  Understand the tidytext framework: Learn how it simplifies the processing, analysis, and visualization of text data.
2.  Master importing techniques: Write functions to efficiently import multiple files into R. Merge data seamlessly: Combine multiple files into a single, coherent data frame.
3.  Clean and manipulate text data: Utilize the tidyverse approach to streamline your text data wrangling.
4.  Create engaging visualizations: Generate captivating Word Clouds to visualize key words.
5.  Calculate TF-IDF: Compute Term Frequency-Inverse Document Frequency using tidytext methods.
6.  Explore text relationships: Visualize the connections between texts and terms to gain deeper insights.
...
-   <div>
    ...
    > 1.  tidytext, tidyverse (mainly readr, purrr, stringr, ggplot2)
    > 2.  widyr
    > 3.  wordcloud and ggwordcloud
    > 4.  textplot (required igraph, tidygraph and ggraph, )
    > 5.  DT
    > 6.  lubridate and hms
...
```{r}
news20 <- "chap29/20news"
```

## 1.4 Define a function to read all files from a folder into a data frame
...
## 2.1 Reading in all the messages from the 20news folder
...
raw_text <- tibble(folder = 
  ...
  transmute(newsgroup = basename(folder), 
            id, text)
write_rds(raw_text, "chap29/news20.rds")
```
...
```{r}
news20 <- readRDS("chap29/news20.rds")
raw_text <- news20
raw_text %>%
  group_by(newsgroup) %>%
...
-   Much of the infrastructure needed for text mining with tidy data frames already exists in packages like ‘dplyr’, ‘broom’, ‘tidyr’, and ‘ggplot2’. Figure below shows the workflow using tidytext approach for processing and visualising text data.

![](images/image10-1.jpg)

## 3.1 Removing header and automated email signatures
...
## 3.2 Removing lines with nested text representing quotes from other users.
...
n this code chunk below, unnest_tokens() of tidytext package is used to split the dataset into tokens, while stop_words() is used to remove stop-words.
...
## 3.4 Visualising Words in newsgroups
...
## 3.5 Visualising Words in newsgroups
...
max_words_per_cloud <- 10  # Set the maximum number of words per cloud

words_by_newsgroup %>%
  filter(n > 0) %>%
  group_by(newsgroup) %>%
  slice_max(order_by = n, n = max_words_per_cloud) %>%
...
### 3.6.1 Computing tf-idf within newsgroups
...
### 3.6.2 Visualising tf-idf as interactive table

Table below is an interactive table created by using datatable().

### 3.6.3 Visualising tf-idf as interactive table
...
## 3.7 Visualising tf-idf within newsgroups Facet bar charts technique is used to visualise the tf-idf values of science related newsgroup.
...
# 4. Counting and correlating pairs of words with the widyr package
...
# 5. Visualising correlation as a network
...
## 6.4Create a network graph from bigram data frame
...
## 6.5 Visualizing a network of bigrams with ggraph
...
-   [Lesson 5: Visualising and Analysing Text Data](https://isss608-ay2023-24apr.netlify.app/lesson/lesson05/lesson05-textvis) slides.
